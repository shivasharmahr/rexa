...
(function initTypewriter(){
  ...
  if(!el) return;
  const lines = ['More than', 'a broker.', 'Your risk', 'partner.'];
  const spans = [];
  lines.forEach((text, i) => {
    if(i > 0) el.appendChild(document.createElement('br'));
    const s = document.createElement('span');
    if(i >= 2) s.style.cssText = 'color:inherit'; // match original <span> styling
    el.appendChild(s);
    spans.push({ s, text });
  });
  ...
  function tick(){
    ...
    if(ci < text.length){
      ...
      li++; ci = 0;
      if(li < spans.length) setTimeout(tick, 220);
